...
var fillBoxes = function fillBoxes() {
  var height = document.querySelector('.game').offsetHeight;

  for (var i = 0; height <= 240; i++) {
...
var calcBorder = function calcBorder(data) {
  totalBorders = data.flatMap(function (el) {
    var curBorderCollections = [el.start];
    var temp = el.start;

    for (var i = 0; i < el.end; i++) {
...
var drawBorder = function drawBorder(borderNums) {
  borderNums.forEach(function (el) {
    var box = document.querySelector("div[data-num=\"".concat(el, "\""));
    box.classList.add('game__border');
    box.dataset.isBorder = true;
  });
};

var createInsect = function createInsect() {
...
var updatePos = function updatePos(start, end, isRow) {
  if (isRow) {
...
var moveInsect = function moveInsect(dir) {
  ...
  ins.style.gridRowStart = "".concat(curPos.row.start);
  ins.style.gridRowEnd = "".concat(curPos.row.end);
  ins.style.gridColumnStart = "".concat(curPos.col.start);
  ins.style.gridColumnEnd = "".concat(curPos.col.end);
};

var addMoveEvents = function addMoveEvents() {
  controller.addEventListener('click', function (e) {
    moveInsect(e.target.dataset.key);
  });
  document.addEventListener('keydown', function (e) {
    if (e.key === 'ArrowUp' || e.key === 'ArrowDown' || e.key === 'ArrowLeft' || e.key === 'ArrowRight') moveInsect(e.key);
  });
};
...
fillBoxes(); // Calculating the borders of provided data

calcBorder(borderData[0].map); // Drawing the borders

drawBorder(totalBorders); // Creating insect that will move

createInsect(); // Adding movement events listeners to the insect
